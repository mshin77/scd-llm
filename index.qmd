...
::: {.iframe-container style="width: 100%; max-width: 800px; height: 1000px; margin: 0 auto;"}
...
```{r, eval=FALSE}
topic_models <- map(unique(new_word_count_data$Topic), function(topic_id) {
  topic_data <- filter(new_word_count_data, Topic == topic_id)
  
  brm(Count ~ 1 + year_init_c + jump_2010 + slope_2010,
      data = topic_data,
...
names(topic_models) <- unique(new_word_count_data$Topic)

model_new_word_results <- map_dfr(names(topic_models), function(topic_id) {
  coefs <- fixef(topic_models[[topic_id]])
  data.frame(
    Topic = topic_id,
    Variable = rownames(coefs),
    Estimate = round(coefs[,"Estimate"], 3),
    Est.Error = round(coefs[,"Est.Error"], 3),
    Q2.5 = round(coefs[,"Q2.5"], 3),
    Q97.5 = round(coefs[,"Q97.5"], 3),
    IRR = round(exp(coefs[,"Estimate"]), 3),
    IRR_Q2.5 = round(exp(coefs[,"Q2.5"]), 3),
    IRR_Q97.5 = round(exp(coefs[,"Q97.5"]), 3),
    stringsAsFactors = FALSE
  )
}) %>%
  arrange(Topic, Variable)
```
...
```{r}
#| code-fold: false
posterior_summary <- posterior_summary(model_new_word)
loo_result <- loo(model_new_word)
pp_check(model_new_word, ndraws = 100)
```
