...
new_word_plot <- plotly::ggplotly(new_word_gg, height = 700, width = 1100) %>% 
  plotly::layout(
    legend = list(
      orientation = "h",
      x = 0.5,
...
#### Fixed Effects (Log Coefficients)

```{r, echo=FALSE}
#| code-fold: false
round(fixef(model_new_word), 2)
```
...
```{r, eval=FALSE}
#| code-fold: false
round(exp(fixef(model_new_word)[, -2]), 2)
```
