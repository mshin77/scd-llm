...
process_tokens <- function(tokens) {
  ...
  tibble::tibble(
    ...
    Min                   = min(freq),
    Q1                    = as.numeric(quantile(freq, 0.25)),
    Median                = median(freq),
    Mean                  = round(mean(freq), 2),
    Q3                    = as.numeric(quantile(freq, 0.75)),
    Max                   = max(freq),
    Skew                  = skew_val,
    Kurt                  = kurt_val
...
decade_stats <- purrr::map_dfr(decades, function(d) {
  ...
  stats %>%
    mutate(
      Decade = d,
      All_Count = Total_Word_Count,
      Reused_Count = sum(reused_tab),
      New_Count = sum(new_tab),
      Topic_Count = nrow(topic_stats),
      Top_10_Reused_Words = if (length(reused_tab)) 
        paste(names(head(reused_tab, 10)), collapse = ", ")
      else 
        NA_character_,
      Top_10_New_Words = if (length(new_tab)) 
...
save(new_word_count_df, new_word_plot, word_usage_summary, model_new_word, model_new_word_results,
     new_word_gg, overall_means, topic_means, file = "files/bayes_model.RData")
```
