...
new_word_count_data <- new_word_count_df %>%
  group_by(Topic) %>%
  mutate(
    year_init_c = Year - min(Year, na.rm = TRUE),
    jump_2010 = as.integer(Year >= 2010),
...
per_topic_effects <- hyp$hypothesis %>%
  ...
  arrange(as.integer(str_extract(Topic, "^\\d+")), factor(Variable, levels = c("Intercept", "Trend before 2010", "Level change in 2010", "Trend change since 2010")))
