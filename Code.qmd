...
model_new_word_results <- coefs %>%
  ...
  mutate(
    ...
    sort_key = case_when(
      ...
      str_detect(Variable, "^Topic\\d+$") ~ paste0("2_", sprintf("%02d", topic_num)),
      str_detect(Variable, "year_init_c") & !str_detect(Variable, ":") ~ "3",
      str_detect(Variable, "year_init_c:") ~ paste0("4_", sprintf("%02d", topic_num)),
      str_detect(Variable, "jump_2010") & !str_detect(Variable, ":") ~ "5",
      str_detect(Variable, "jump_2010:") ~ paste0("6_", sprintf("%02d", topic_num)),
      str_detect(Variable, "slope_2010") & !str_detect(Variable, ":") ~ "7",
      str_detect(Variable, "slope_2010:") ~ paste0("8_", sprintf("%02d", topic_num)),
      TRUE ~ "9"
    )
  ) %>%
  arrange(sort_key) %>%
  select(-topic_num, -sort_key)

model_new_word_results_apa <- model_new_word_results %>%
  mutate(
    `Estimate (SD)` = sprintf("%.2f (%.2f)", Estimate, Est.Error),
    `Estimate 95% CI` = sprintf("[%.2f, %.2f]", Q2.5, Q97.5),
    `IRR` = sprintf("%.2f", IRR),
    ...
    Variable = case_when(
      ...
      str_detect(Variable, "^Topic\\d+$") ~ paste("Topic", str_extract(Variable, "\\d+")),
      Variable == "year_init_c" ~ "Trend before 2010",
      Variable == "jump_2010" ~ "Level change in 2010",
      Variable == "slope_2010" ~ "Trend change since 2010",
      str_detect(Variable, "year_init_c:Topic") ~ paste("Trend before 2010 × Topic", str_extract(Variable, "\\d+$")),
      str_detect(Variable, "jump_2010:Topic") ~ paste("Level change in 2010 × Topic", str_extract(Variable, "\\d+$")),
      str_detect(Variable, "slope_2010:Topic") ~ paste("Trend change since 2010 × Topic", str_extract(Variable, "\\d+$")),
      TRUE ~ Variable
    )
  ) %>%
  select(Variable, `Estimate (SD)`, `Estimate 95% CI`, IRR, `IRR 95% CI`, Sig)
...
```{r, echo=FALSE}
loo_piecewise <- loo(model_new_word, moment_match = TRUE)
print(loo_piecewise)
```

#### Topic-Specific Piecewise Effects
...
hyp_strings <- c(
  ...
  paste0("year_init_c + year_init_c:Topic", topics_other, " = 0"),
  paste0("jump_2010 + jump_2010:Topic", topics_other, " = 0"),
  paste0("slope_2010 + slope_2010:Topic", topics_other, " = 0")
)
...
effect_vec <- c(
  "Intercept", "Trend before 2010", "Level change in 2010", "Trend change since 2010",
  rep("Intercept", n_other),
  rep("Trend before 2010", n_other),
  rep("Level change in 2010", n_other),
  rep("Trend change since 2010", n_other)
)
...
per_topic_effects <- hyp$hypothesis %>%
  ...
  mutate(
    ...
    `Estimate (SD)` = sprintf("%.2f (%.2f)", Estimate, Est.Error),
    `Estimate 95% CI` = sprintf("[%.2f, %.2f]", CI.Lower, CI.Upper),
    `IRR` = sprintf("%.2f", IRR),
    `IRR 95% CI` = sprintf("[%.2f, %.2f]", IRR_lower, IRR_upper)
  ) %>%
  select(Topic, Effect, `Estimate (SD)`, `Estimate 95% CI`, IRR, `IRR 95% CI`, Sig) %>%
  arrange(as.integer(str_extract(Topic, "^\\d+")), factor(Effect, levels = c("Intercept", "Trend before 2010", "Level change in 2010", "Trend change since 2010")))
