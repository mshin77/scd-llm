...
[This website](https://mshin77.github.io/scd-llm/) contains `Data` and `R and Python code` used for the analyses in Shin and McKenna (in press). The scripts were also posted through an online data repository at [Center for Open Science](http://osf.io/fyajc) and [GitHub](https://github.com/mshin77/scd-llm).
...
![Figure 1. Semi-Automated Publication Screening](files/screening_procedure.png)
...
![Figure 2 (a). Training Loss and Accuracy](files/single-case-results.png)

![](files/confusion_roc_single_case.png)
...
![](files/confusion_roc_technology_use.png)
...
[Employed a list of **77 wildcard expressions** using **regular expressions** to filter methodology content. See Code for the full list.]{style="background-color: #EBECF0; padding: 5px; border-left: 3px solid #949494; display: inline-block; margin: 10px 0;"}
...
topic_starts <- new_word_count_df %>%
  group_by(Topic) %>%
  summarise(First_Year = as.character(min(Year, na.rm = TRUE)), .groups = "drop")
```
