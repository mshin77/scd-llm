...
model_new_word_results_apa <- model_new_word_results %>%
  mutate(
    ...
    Variable = case_when(
      ...
      Variable == "year_init_c" ~ "Year (centered)",
      Variable == "jump_2010" ~ "Jump at 2010",
      Variable == "slope_2010" ~ "Slope change at 2010",
      str_detect(Variable, "year_init_c:Topic") ~ paste("Year × Topic", str_extract(Variable, "\\d+$")),
      str_detect(Variable, "jump_2010:Topic") ~ paste("Jump 2010 × Topic", str_extract(Variable, "\\d+$")),
      str_detect(Variable, "slope_2010:Topic") ~ paste("Slope 2010 × Topic", str_extract(Variable, "\\d+$")),
      TRUE ~ Variable
    )
  ) %>%
...
effect_vec <- c(
  "Intercept", "Pre-2010 slope", "Jump at 2010", "Post-2010 slope change",
  rep("Intercept", n_other),
  rep("Pre-2010 slope", n_other),
  rep("Jump at 2010", n_other),
  rep("Post-2010 slope change", n_other)
)
...
per_topic_effects <- hyp$hypothesis %>%
  ...
  arrange(as.integer(str_extract(Topic, "^\\d+")), factor(Effect, levels = c("Intercept", "Pre-2010 slope", "Jump at 2010", "Post-2010 slope change")))
