---
title: 'Exploring the Landscape of Technology Use in Single-Case Design Research Methodology Through Text Mining With Large Language Models'
# logo: ''
date: "`r Sys.Date()`"
...
wos_1_500 <- convert2df("../wos/1_500.ciw", dbsource = "wos", format = "endnote")
wos_501_1000 <- convert2df("../wos/501_1000.ciw", dbsource = "wos", format = "endnote")
wos_1001_1500 <- convert2df("../wos/1001_1500.ciw", dbsource = "wos", format = "endnote")
wos_1501_2000 <- convert2df("../wos/1501_2000.ciw", dbsource = "wos", format = "endnote")
wos_2001_2500 <- convert2df("../wos/2001_2500.ciw", dbsource = "wos", format = "endnote")
wos_2501_3000 <- convert2df("../wos/2501_3000.ciw", dbsource = "wos", format = "endnote")
wos_3001_3500 <- convert2df("../wos/3001_3500.ciw", dbsource = "wos", format = "endnote")
wos_3501_4000 <- convert2df("../wos/3501_4000.ciw", dbsource = "wos", format = "endnote")
wos_4001_4500 <- convert2df("../wos/4001_4500.ciw", dbsource = "wos", format = "endnote")
wos_4501_4860 <- convert2df("../wos/4501_4860.ciw", dbsource = "wos", format = "endnote")
```
...
data_combined <- data %>% left_join(df, by = "UT")

data <- read_excel("../data/data_combined.xlsx")

setDT(data)

clean_reference <- function(refs) {
  
  refs_cleaned <- gsub("&#\\d+;", "", refs)
  refs_cleaned <- gsub("\\d+\\.\\d+\\.\\p{L}+", "", refs_cleaned, perl = TRUE)
  refs_cleaned <- stri_replace_all_regex(refs_cleaned, "\\s+", " ")
  refs_cleaned <- trimws(refs_cleaned)
...
ref_data <- ref_data[reference != "" & !is.na(reference)]

reference_freq <- ref_data[, .N, by = reference]

ref_data[, reference := gsub(",\\s*doi.*$", "", reference, ignore.case = TRUE)]

ref_data <- ref_data[!grepl("\\bAnonymous\\b", reference, ignore.case = TRUE) & 
                     !grepl("^\\d+$", reference)]

ref_data[, reference := gsub("\\s+", " ", reference)]
ref_data[, reference := trimws(reference)]

ref_data <- ref_data[!duplicated(ref_data[, .(doc_id, reference)]), ]

doc_ref_counts <- ref_data[, .N, by = doc_id]

reference_freq <- ref_data[, .N, by = reference]

duplicate_refs_across_docs <- reference_freq[N > 1]

co_citation_pairs <- ref_data[, {
  if (.N > 1) {
...
reference_to_group <- setNames(ref_data$group, ref_data$reference)
...
```{r}
co_citation_network <- visNetwork(nodes, edges, height = "690px", width = "100%") %>%
  visNodes(
    color = list(
      background = ~color,
      border = "black"
...
# htmlwidgets::saveWidget(co_citation_network, "../results/co_citation_network/index.html")
htmlwidgets::saveWidget(co_citation_network, "../results/index.html")
```
